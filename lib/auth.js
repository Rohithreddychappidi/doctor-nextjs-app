...
export async function requireAdmin() {
  ...
  const isEmailConfigured =
    ...
    assistantEmails.includes(userEmail);

  // Dual-Authority Database Verification:
  // Never trust environment variables alone. Check persistent DB records.
  const dbUser = await db.findUserByEmail(userEmail);
  const dbRoles = dbUser ? await db.getUserRoles(dbUser.id) : (user.roles || [user.role]);

  const hasAdminRole =
    dbRoles.includes("super_admin") ||
    dbRoles.includes("admin") ||
    dbRoles.includes("sub_admin");

  if (!isEmailConfigured || !hasAdminRole) {
    if (isEmailConfigured && !hasAdminRole) {
      console.error(`[SECURITY ALERT] Admin email configured without DB admin role: ${userEmail}`);
    }
    throw new Error("Forbidden: Access restricted to authorized administrator with verified authority.");
  }
  return user;
}
