...
export const memoryStore = {
  ...
  users: [
    {
      id: "usr_admin_jvm",
      email: (process.env.ADMIN_EMAIL || "admin@mydam-medical.com").toLowerCase().trim(),
  ...
  // 8b. Question Bank Subject Pillars (The 3 Core Disciplines)
  qbank_specializations: [
    {
      id: "spec_neo",
      code: "NEO",
      name: "Neonatology & Perinatal Medicine",
      description: "NICU delivery room resuscitation (NRP), surfactant kinetics, PPHN, extreme prematurity, and neonatal hemodynamics.",
      icon: "👶",
      total_modules: 0
    },
    {
      ...
      description: "Outpatient and inpatient pediatric clinical vignettes, immunization schedules, milestones, cardiology, and pediatric board review.",
      icon: "🩺",
      total_modules: 0
    },
    {
      ...
      description: "Clinical study designs, sensitivity/specificity, likelihood ratios, NNT, survival analysis, and evidence-based medicine appraisal.",
      icon: "📊",
      total_modules: 0
    }
  ],

  // 8c. Question Bank Modules (Dynamic modules authored by Dr. Janardhan Mydam via Admin Panel)
  qbank_modules: [
    {
      id: "mod_nrp_resuscitation",
      specialization_id: "spec_neo",
      name: "Delivery Room NRP 8th Edition & Acute Resuscitation",
      description: "Master ventilation corrective steps (MR. SOPA), chest compression ratios, UVC emergency access, and epinephrine administration.",
      is_free: true,
      price: 0,
      created_at: new Date().toISOString()
    },
    {
      id: "mod_rds_surfactant",
      specialization_id: "spec_neo",
      name: "Prematurity, Surfactant Kinetics & BPD Prevention",
      description: "Non-invasive CPAP thresholds, LISA surfactant delivery techniques, and ventilatory weaning strategies in ELBW infants.",
      is_free: false,
      price: 15,
      created_at: new Date().toISOString()
    },
    {
      id: "mod_peds_milestones",
      specialization_id: "spec_peds",
      name: "Developmental Milestones & AAP Immunization Schedules",
      description: "High-yield developmental screening, gross motor, language milestones, vaccine contraindications, and well-child visits.",
      is_free: true,
      price: 0,
      created_at: new Date().toISOString()
    },
    {
      id: "mod_peds_sepsis",
      specialization_id: "spec_peds",
      name: "Pediatric Sepsis, Febrile Infant & Inpatient Emergencies",
      description: "Rochester/Philadelphia criteria for young infants, empiric antimicrobial regimens, and septic shock vasoactive titration.",
      is_free: false,
      price: 15,
      created_at: new Date().toISOString()
    },
    {
      id: "mod_biostats_testing",
      specialization_id: "spec_biostats",
      name: "Diagnostic Tests, Sensitivity, Specificity & Bayes Nomograms",
      description: "2x2 table calculations, positive and negative likelihood ratios, pre-test and post-test probability calculations.",
      is_free: true,
      price: 0,
      created_at: new Date().toISOString()
    },
    {
      id: "mod_biostats_trials",
      specialization_id: "spec_biostats",
      name: "Clinical Trial Designs, Bias, NNT, NNH & Power Analysis",
      description: "Cohort vs case-control study design, selection and recall bias, intention-to-treat analysis, and Type I/II errors.",
      is_free: false,
      price: 15,
      created_at: new Date().toISOString()
    }
  ...
  rotation_announcements: [
    {
      id: "ann_rot_1",
      cohort_id: "cohort_fall_2026",
      title: "Orientation Class & Microsoft Teams Setup",
      message: "Clinical Tele-Rotation Orientation will be held this Monday at 10:00 AM CST. Please verify your Microsoft Teams credentials and ensure your video cameras are operational for live bedside simulation.",
      author: "Dr. Janardhan Mydam, MD, FAAP",
      link: "https://teams.microsoft.com",
      attachments: ["Orientation_Checklist.pdf"],
      created_at: "2026-09-10T14:00:00Z"
    },
    {
      id: "ann_rot_2",
      cohort_id: "cohort_fall_2026",
      title: "Upcoming Grand Rounds: PPHN & Nitric Oxide Titration",
      message: "Thursday morning rounds will focus on Persistent Pulmonary Hypertension of the Newborn (PPHN). Please review the echocardiogram Doppler criteria prior to the roundtable discussion.",
      author: "Dr. Janardhan Mydam, MD, FAAP",
      link: "",
      attachments: ["PPHN_Echo_Criteria.pdf"],
...
export const db = {
  ...
  async unlockQBankModule(studentId, moduleId, paymentData = {}) {
    ...
    memoryStore.student_qbank_unlocks.push(record);
    return record;
  },

  async getStudentQBankUnlocks(studentId) {
    if (!memoryStore.student_qbank_unlocks) memoryStore.student_qbank_unlocks = [];
    return memoryStore.student_qbank_unlocks.filter((u) => u.student_id === studentId);
  },
  ...
  // 25. CMS Content Management (Public Navbar Sections & Content)
  async getCmsData(key) {
    if (!key) return null;
    const p = getPool();
    if (p) {
      try {
        const res = await p.query("SELECT value FROM system_settings WHERE key = $1 LIMIT 1", [`cms_${key}`]);
        if (res?.rows?.[0]) return res.rows[0].value;
      } catch (_) {
        // Fallback to memoryStore
      }
    }
    if (!memoryStore.cms_content) memoryStore.cms_content = {};
    return memoryStore.cms_content[key] || null;
  },

  async saveCmsData(key, data) {
    if (!key) return null;
    const p = getPool();
    if (p) {
      try {
        await p.query(
          ...
          [`cms_${key}`, JSON.stringify(data)]
        );
      } catch (err) {
        console.warn("PostgreSQL CMS save fallback:", err.message);
      }
    }
    if (!memoryStore.cms_content) memoryStore.cms_content = {};
    memoryStore.cms_content[key] = data;
    return data;
  },
  ...
  async registerStudentForClass(regData) {
    ...
    memoryStore.student_class_registrations.push(reg);
    return reg;
  },

  async getClassRegistrations(classId = null) {
    if (!memoryStore.student_class_registrations) memoryStore.student_class_registrations = [];
    if (classId) {
      return memoryStore.student_class_registrations.filter((r) => r.class_id === classId);
    }
    return memoryStore.student_class_registrations;
  },

  // Rotation Announcements & Messages
  async getRotationAnnouncements(cohortId = null) {
    if (!memoryStore.rotation_announcements) memoryStore.rotation_announcements = [];
    if (cohortId) {
      return memoryStore.rotation_announcements.filter((a) => !a.cohort_id || a.cohort_id === cohortId);
    }
    return memoryStore.rotation_announcements;
  },

  async createRotationAnnouncement(data) {
    if (!memoryStore.rotation_announcements) memoryStore.rotation_announcements = [];
    const newAnn = {
      id: `ann_rot_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`,
      cohort_id: data.cohort_id || data.target_cohort || "cohort_fall_2026",
      title: data.title?.trim() || "Clinical Announcement",
      message: data.message?.trim() || "",
      author: data.author?.trim() || data.posted_by || "Dr. Janardhan Mydam, MD, FAAP",
      priority: data.priority || "Normal",
      category: data.category || "General Clinical Notice",
      action_url: data.action_url || data.link || "",
      requires_ack: Boolean(data.requires_ack),
      link: data.link?.trim() || data.action_url?.trim() || "",
      attachments: Array.isArray(data.attachments) ? data.attachments : data.attachment ? [data.attachment] : [],
      created_at: new Date().toISOString()
    };
    memoryStore.rotation_announcements.unshift(newAnn);
    return newAnn;
  },
  ...
  async createResearchGroup(data) {
    ...
    const newGroup = {
      ...
      members: Array.isArray(data.members) ? data.members : [
        { id: "usr_admin", name: "Dr. Janardhan Mydam, MD, FAAP", email: process.env.ADMIN_EMAIL || "faculty@mydam-medical.com", role: "doctor" }
      ],
      messages: [
        {
          id: `msg_${Date.now()}`,
          sender_name: "Dr. Janardhan Mydam, MD, FAAP",
          sender_role: "doctor",
          sender_email: process.env.ADMIN_EMAIL || "faculty@mydam-medical.com",
  ...
  // General Consultation Requests & Scheduling
  async getConsultationRequests() {
    if (!memoryStore.consultation_requests) memoryStore.consultation_requests = [];
    return memoryStore.consultation_requests;
  },
  ...
  async createConsultationRequest(data) {
    ...
    const newReq = {
      ...
      phone: data.phone || "",
      institution: data.institution || "",
      stage: data.stage || "Medical Candidate",
      focus_area: data.focus_area || "General Mentorship",
      cv_url: data.cv_url || data.cv_link || "",
      preferred_time: data.preferred_time || data.preferredTime || "",
      message: data.message || data.questions || "",
      status: "new", // "new" | "scheduled" | "completed" | "cancelled"
      scheduled_date: null,
      scheduled_time: null,
      meeting_link: null,
      meeting_platform: "Microsoft Teams",
    ...
    memoryStore.consultation_requests.unshift(newReq);
    return newReq;
  },

  async updateConsultationRequest(id, updates) {
    if (!memoryStore.consultation_requests) memoryStore.consultation_requests = [];
    const req = memoryStore.consultation_requests.find((c) => c.id === id);
    if (!req) return null;
